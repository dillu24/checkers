...
func ParseDateAddedAsTime(dateAdded string) (dateAddedParsed time.Time, err error) {
	dateAddedParsed, err = time.Parse(DateAddedLayout, dateAdded)
	return dateAddedParsed, sdkerrors.Wrapf(err, ErrInvalidDateAdded.Error(), dateAdded)
}
...
func ParseWinners(winners []WinningPlayer) (parsedWinners []WinningPlayerParsed, err error) {
	...
	for index, winner := range winners {
		parsedWinner, err := winner.Parse()
		if err != nil {
			return nil, err
		}
		parsedWinners[index] = parsedWinner
	}
	return parsedWinners, nil
}

func (leaderboard Leaderboard) ParseWinners() ([]WinningPlayerParsed, error) {
...
func UpdatePlayerInfoAtNow(
	winners []WinningPlayerParsed,
	now time.Time,
	candidate PlayerInfo,
) (updated []WinningPlayerParsed) {
