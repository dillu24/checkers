...
func TestPlayMoveUpToWinner(t *testing.T) {
	...
	event := events[0]
	require.Equal(t, types.MovePlayedEventType, event.Type)
	require.EqualValues(
		t,
		[]sdk.Attribute{
			{Key: types.MovePlayedEventCreator, Value: bob},
			{Key: types.MovePlayedEventGameIndex, Value: "1"},
			{Key: types.MovePlayedEventCapturedX, Value: "2"},
			{Key: types.MovePlayedEventCapturedY, Value: "5"},
			{Key: types.MovePlayedEventWinner, Value: "b"},
