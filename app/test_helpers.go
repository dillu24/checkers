...
func SetupWithGenesisValSet(
	t *testing.T, valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount,
	balances ...banktypes.Balance) *App {
	...
	for _, val := range valSet.Validators {
		...
		delegations = append(delegations, stakingtypes.NewDelegation(
			genAccs[0].GetAddress(), val.Address.Bytes(), sdk.OneDec()))
...
func addTestAddrs(app *App, ctx sdk.Context, accNum int, accAmt sdk.Int,
	strategy GenerateAccountStrategy) []sdk.AccAddress {
...
// every transaction.
func GenSequenceOfTxs(txGen client.TxConfig, msgs []sdk.Msg, accNums []uint64, initSeqNums []uint64, numToGenerate int,
...
func CreateTestPubKeys(numPubKeys int) []cryptotypes.PubKey {
	...
	for i := 100; i < (numPubKeys + 100); i++ {
		...
		buffer.WriteString(numString)                                                       // adding on final two
		// digits to make pubkeys
		//unique
